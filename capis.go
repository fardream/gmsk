...
// Dot wraps MSK_dot and performs a dot product of two vectors
func (env *Env) Dot(n int32, x, y *float64) (r res.Code, xty float64) {
	r = res.Code(
		C.MSK_dot(
			env.getEnv(),
			mi32(n),
...
// Axpy wraps MSK_axpy and performs y = a*x + y where x/y are vectors
func (env *Env) Axpy(n int32, alpha float64, x, y *float64) res.Code {
	return res.Code(
		C.MSK_axpy(
			env.getEnv(),
			mi32(n),
...
// Syeig wraps MSK_syeig and calculates the eigen values of a symmetric matrix.
func (env *Env) Syeig(uplo UpLo, n int32, a *float64, w *float64) res.Code {
	return res.Code(
		C.MSK_syeig(
			env.getEnv(),
			C.MSKuploe(uplo),
...
// Syevd wraps MSK_syevd and calculates the eigen values and eigen vectors of a symmetric matrix.
func (env *Env) Syevd(uplo UpLo, n int32, a *float64, w *float64) res.Code {
	return res.Code(
		C.MSK_syevd(
			env.getEnv(),
			C.MSKuploe(uplo),
